...
def search_recipes_spoonacular(ingredients: List[str], limit: int = 10):
    ...
    try:
        ...
        print(f"Making request to Spoonacular: {url} with params: {params}")
        response = requests.get(url, params=params)
        response.raise_for_status()
        
        recipes_data = response.json()
        print(f"Received {len(recipes_data)} recipes from Spoonacular")
...
@app.get("/api/test")
def test_endpoint():
    return {"message": "API server working"}
...
def search_recipes_endpoint(request: SearchRequest):
    ...
    try:
        ...
        print(f"Error in search_recipes_endpoint: {e}")
        raise HTTPException(status_code=500, detail="Error searching for recipes")
